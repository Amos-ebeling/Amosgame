...
void AssetManager::get_available_items(const std::string& filename, Graphics& graphics, World& world) {
    ...
    // sheep
    auto sheep_json = json.at("sheep");
    std::string name = sheep_json.at("name").get<std::string>();
    Physics physics = sheep_json.at("physics").get<Physics>();
    Vec<int> size = sheep_json.at("size").get<Vec<int> >();
    int damage = sheep_json.at("damage").get<int>();
    int health = sheep_json.at("health").get<int>();
    int max_health = sheep_json.at("max_health").get<int>();

    std::vector<Sprite> sprites_from_json = sheep_json.at("sprites").get<std::vector<Sprite> >();
    //build the sprites
    Sheep tmp{name, nullptr, nullptr};
    convert_sprites(sprites_from_json, graphics, &tmp, false);
    auto sprites = tmp.sprites;

    world.available_items[name] = [name, physics, sprites, size, damage, health, max_health]() {
        auto sheep = new Sheep{name, nullptr, nullptr};
        sheep->physics = physics;
        sheep->sprites = sprites;
        sheep->size = size;
        sheep->damage = damage;
        sheep->health = health;
        sheep->max_health = max_health;
        sheep->set_sprite("idle");
        return sheep;
    };
}
